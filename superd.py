...
# >>> superd.write_model(m, 15, '/tmp/trebol.stl')
# >>> superd.write_boundaries(m, 50, '/tmp/trebol-boundaries.obj')
...
def eval_bezier_curve(cp, u):
    """Evaluates a Bezier surface at parameter `u`.
    The control polygon is given as a list of 3D points.
    """
    order = len(cp)
    coeff = bernstein(order - 1, u)
    result = np.array([0., 0, 0])
    for i in range(order):
        result += cp[i] * coeff[i]
    return result

def eval_bezier_surface(cp, u, v):
...
def eval_patch_impl(domain, ribbons, p):
    ...
    blendsum = sum(blends)
    for i in range(n):
        result += eval_bezier_surface(ribbons[i], s[i], d[i]) * blends[i] / blendsum
    return result
...
def write_boundaries(model, resolution, filename):
    """Writes the boundary curves in `model` into an OBJ file.
    Note that curves between multiple patches are written multiple times.
    """
    with open(filename, 'w') as f:
        sizes = []
        for patch in model:
            n = patch['n']
            sizes.append(n * resolution)
            for i in range(n):
                base = generate_base(patch, i)
                for j in range(resolution):
                    u = j / resolution
                    p = eval_bezier_curve(base, u)
                    f.write('v {0} {1} {2}\n'.format(p[0], p[1], p[2]))
        sum = 0
        for s in sizes:
            for i in range(1, s):
                f.write('l {0} {1}\n'.format(sum + i, sum + i + 1))
            f.write('l {0} {1}\n'.format(sum + s, sum + 1))
            sum += s
